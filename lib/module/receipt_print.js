...
import { BarcodeSymbology, DividerStyle, FontFamily, ImageSourceType, TextAlignment, TypefaceStyle } from "./enums/printer.js";
const DEFAULT_STYLE = {
...
function resolve_style(overrides) {
  return {
    ...DEFAULT_STYLE,
    ...overrides
  };
...
export class ReceiptPrintBuilder {
   ...
   */
  addColumns(columns, style) {
    const resolved_columns = columns.map((col, index) => {
   ...
   */
  addDivider(char = '-', style = DividerStyle.SOLID) {
    this.nodes.push({
      type: 'divider',
      char,
      style
  ...
  * @returns {this} Para encadeamento de chamadas.
  */
  addImage(source, sourceType = ImageSourceType.BASE64, options) {
    this.nodes.push({
      type: 'image',
      source,
      sourceType,
   ...
   */
  addBarcode(data, symbology = BarcodeSymbology.CODE128, height = 60) {
    this.nodes.push({
      type: 'barcode',
      data,
      symbology,
...
//# sourceMappingURL=receipt_print.js.map