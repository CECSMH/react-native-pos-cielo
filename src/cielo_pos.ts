...
import { parse_terminal_info } from "./interfaces/terminal";

import PosCielo from './NativePosCielo';
import type { PaymentMethod } from "./enums/payment.ts";
import { parse_cielo_error, ValidationError } from "./interfaces/errors";
import {
    PaymentResult,
    parse_cancel_result,
...
import { validate_print_layout, type ReceiptLayout } from "./interfaces/printer.js";
...
export default class Cielo {
     ...
     */
    static async is_running_on_pos(): Promise<boolean> {
        try {
     ...
     * @throws {CieloError}
     */
    static async get_terminal_info(): Promise<TerminalInfo> {
        try {
     ...
     * @throws {CieloError}
     */
    static async get_available_payments(): Promise<PaymentMethod[]> {
        try {
            const j_str = await PosCielo.getProductList();

            return JSON.parse(j_str) as PaymentMethod[];
        } catch (error) {
            throw parse_cielo_error(error);
        };
    };
     ...
     */
    static async request_payment(request: PaymentRequest): Promise<PaymentResult> {
        try {
    ...
    * @throws {ValidationError}
    */
    static async cancel_payment(request: CancelRequest) {
        try {
     ...
     * @throws {CieloError}
     */
    static async print(input: ReceiptLayout | ReceiptPrintBuilder): Promise<void> {
        try {
...
function is_receipt_builder(input: unknown): input is ReceiptPrintBuilder {
